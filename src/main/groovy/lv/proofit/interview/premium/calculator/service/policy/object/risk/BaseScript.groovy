class BaseScript extends Script {
	...
	ArrayList<LinkedHashMap<String, Serializable>> getAgeFactorData() {
		return [['MAKE': 'Canyon', 'MODEL': 'CF 5', 'VALUE_FROM': 0, 'VALUE_TO': 5, 'FACTOR_MIN': 1.5, 'FACTOR_MAX': 2],
				...
				['VALUE_FROM': 0, 'VALUE_TO': 15, 'FACTOR_MIN': 1.0, 'FACTOR_MIN': 3.0]]
	}
	...
	ArrayList<LinkedHashMap<String, Serializable>> getRiskCountFactorData() {
		return [
				...
				['VALUE_FROM': 5, 'VALUE_TO': 10, 'FACTOR_MIN': 1.0, 'FACTOR_MAX': 1.0]
		]
	}

	ArrayList<LinkedHashMap<String, Serializable>> getSumInsuredFactorData() {
		return [
				['VALUE_FROM': 100, 'VALUE_TO': 1000, 'FACTOR_MIN': 0.5, 'FACTOR_MAX': 1.0],
				['VALUE_FROM': 1001, 'VALUE_TO': 3000, 'FACTOR_MIN': 1.0, 'FACTOR_MAX': 2.0],
				['VALUE_FROM': 3001, 'VALUE_TO': 5000, 'FACTOR_MIN': 2.0, 'FACTOR_MAX': 3.0]
	...
	ArrayList<LinkedHashMap<String, Serializable>> getRiskBasePremiumData() {
		return [
				['RISK_TYPE': 'DAMAGE', 'PREMIUM': 10.0],
				['RISK_TYPE': 'THIRD_PARTY_DAMAGE', 'PREMIUM': 20.0],
				['RISK_TYPE': 'THEFT', 'PREMIUM': 30.0]
		]
	}
}
